...
* History: 브라우저의 history와 비슷하다. 하지만 스택에 현재까지 이동한 경로가 담겨있는 형태로 주소를 임의로 변경하거나 되돌릴 수 있다.
...
라우팅은 기본적으로 네트워크에서 경로를 선택하는 프로세스를 의미하며, 다양한 주소의 요청이 들어왔을 때 각각 맞는 콘텐츠로 이동시켜 주는 작업이다. 마치 우체국에서 편지를 집 주소에 맞게 배달하는 것처럼, 웹 애플리케이션에서는 URL 경로에 따라 다른 콘텐츠나 페이지를 표시한다.
