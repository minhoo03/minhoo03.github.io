...
이유는 ```이벤트 루프(Event Loop)```가 싱글쓰레드이기 때문이다. 대신 웹 브라우저나 NodeJs와 같은 멀티쓰레드 환경에서 이를 사용함으로 멀티쓰레드처럼 사용할 수 있던 것이다.

<!--
JS 동작 과정 + JS Engine + Memory Heap & Call Stack + Callback Queue + 이벤트 루프
-->
